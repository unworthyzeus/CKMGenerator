...
- LoS from the refined coherent two-ray prior in
...
def predict_nlos_map_torch(
    ...
    for ab_try in (ab, "mid_ant", "low_ant", "high_ant"):
        ...
        if key in coefs:
            ...
            pred = torch.matmul(x_flat.cpu(), w.cpu()).to(device).reshape(prior.shape)
...
def main() -> None:
    ...
    parser.add_argument("--new-calibration-json", type=Path, default=CALIB_DIR / "try78_los_two_ray_calibration.json")
    parser.add_argument("--old-calibration-json", type=Path, default=CALIB_DIR / "try78_nlos_regime_calibration.json")
